...
impl MqttState {
    ...
    pub fn outgoing_publish(&mut self, publish: Publish) -> Result<(), StateError> {
    ...
    pub fn handle_outgoing_packet(&mut self, request: Packet) -> Result<(), StateError> {
        match request {
            ...
            Packet::PingReq => self.outgoing_ping()?,
            Packet::Disconnect => self.outgoing_disconnect()?,
            _ => unimplemented!(),
        };
